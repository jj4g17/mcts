...
plot.test <- function(x){
  ggplot(data=x$results, aes(x=score, y=Freq/length(x$raw))) +
...
#' @export
summary.test <- function(x){
  cat("The probability of passing such a test at the 40% standard pass rate is", score(x), "\n")
  summary(x$raw)
}
...
#' Calculate probability of scoring over v\% on a test x
#'
#' @param x the input test
#' @param v the threshold score percentage (defaults to 0.40)
#'
#' @return The proportion of students who would score at least v\% by random guessing.
#' @export
#'
